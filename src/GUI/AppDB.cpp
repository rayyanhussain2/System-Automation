...
public:
    ...
    {
        sqlite3_stmt* stmt;
        string sql = "SELECT api_key FROM settings;";
        int rc = sqlite3_prepare_v2(dbPointer, sql.c_str(), -1, &stmt, 0);

        if (rc != SQLITE_OK) {
            return ""; // Return empty string if prepare fails
        }

        if (sqlite3_step(stmt) == SQLITE_ROW) {
            const unsigned char* result = sqlite3_column_text(stmt, 0);
            string api_key = result ? (char*)result : "";
            sqlite3_finalize(stmt);
            return api_key;
        }

        sqlite3_finalize(stmt);
        return ""; // Return empty string if no row found
    }
