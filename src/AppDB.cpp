...
class AppDB{
    ...
    public:
        AppDB(){
            ...
            if (!homeDir) 
                throw 1; // HOME environment variable not found

            string stringDbPath = string(homeDir) + "/.config/SystemAutomation/user.db";
            if(sqlite3_open(stringDbPath.c_str(), &dbPointer) != SQLITE_OK)
        ...
        void insertHistory(string input){
            ...
            if(sqlite3_prepare_v2(dbPointer, sql, -1, &rows, nullptr) == SQLITE_OK){
                while(sqlite3_step(rows) == SQLITE_ROW){
                    rowCount = sqlite3_column_double(rows, 0); //struct rows, column
                }
            }
           
            string localSQL = "INSERT INTO history (id, time, input) VALUES (" + to_string(rowCount) + ", " + "CURRENT_TIMESTAMP, " + "\"" + input + "\"" + ");";
            if(sqlite3_exec(dbPointer, localSQL.c_str(), nullptr, nullptr, &error) != SQLITE_OK){
                cout << error << endl;
                throw 2;
            }
        ...
        vector<vector<string>> fetchHistory(){
            sql = "SELECT * FROM history;";
            vector<vector<string>> result;
            if(sqlite3_prepare_v2(dbPointer, sql, -1, &rows, nullptr) == SQLITE_OK){
                while(sqlite3_step(rows) == SQLITE_ROW){
                    string id = to_string(sqlite3_column_int(rows, 0)); //struct rows, column
                    string timeStamp =  string(reinterpret_cast<const char*>(sqlite3_column_text(rows, 1)));
                    string input = string(reinterpret_cast<const char*>(sqlite3_column_text(rows, 2)));
                    result.push_back({id, timeStamp, input});
                }
            }
        ...
        vector<string> fetchHistoryLatest(){
            sql = "SELECT COUNT(*) FROM history;";
            int rowCount;
            if(sqlite3_prepare_v2(dbPointer, sql, -1, &rows, nullptr) == SQLITE_OK){
                while(sqlite3_step(rows) == SQLITE_ROW){
                    rowCount = sqlite3_column_int(rows, 0); //struct rows, column                }
                }
            }
            
            rowCount -= 1;
            
            string localSQL = "SELECT * FROM history WHERE id = " + to_string(rowCount) + ";";
            if(sqlite3_prepare_v2(dbPointer, localSQL.c_str(), -1, &rows, nullptr) == SQLITE_OK){
                while(sqlite3_step(rows) == SQLITE_ROW){
                    string id = to_string(sqlite3_column_int(rows, 0)); //struct rows, column
                    string timeStamp =  string(reinterpret_cast<const char*>(sqlite3_column_text(rows, 1)));
                    string input = string(reinterpret_cast<const char*>(sqlite3_column_text(rows, 2)));
